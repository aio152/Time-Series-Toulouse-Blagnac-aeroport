...
ljung_box_test <- Box.test(decomp.multi$random, lag = 5, type = "Ljung-Box")
ljung_box_test
...
# ACF et PACF to determine the modle
...
# Our manuelle model is better than the auto arima model 

n_forecast = 8  # Nombre de mois à prévoir
...
# Ajouter les données réelles à partir de 1993
...
mae <- mean(abs(predicted_values - test_data_before))

# Calculer l'erreur quadratique moyenne (RMSE)
rmse <- sqrt(mean((predicted_values - test_data_before)^2))

# Calculer l'erreur absolue moyenne en pourcentage (MAPE)
mape <- mean(abs((predicted_values - test_data_before) / test_data_before)) * 100
...
lines(forecasted_values_2002$lower[,1], col = "black", lty = 2)  # Limite inférieure
lines(forecasted_values_2002$upper[,1], col = "black", lty = 2)  # Limite supérieure
